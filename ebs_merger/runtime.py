...
放在 ebs_merger(底层包)中,使 ebs_merger 与 ifmerge_gui 都能引用而不产生
反向依赖(ifmerge_gui -> ebs_merger 是允许方向)。

两类路径:
  - resource_path():只读资源(prompts.yaml / template / locales)。
...
def _repo_root() -> Path:
    # runtime.py 位于 <root>/ebs_merger/runtime.py,parents[1] = <root>。
