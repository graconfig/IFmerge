"""ebs_merger.runtime 路径解析测试(开发态 + 模拟打包态)。"""

from pathlib import Path

from ebs_merger import runtime
...
def test_bundled_resources_resolve_to_existing_files_in_dev():
    ...
    assert runtime.resource_path("template", "IF_Template.xlsm").exists()
    assert runtime.resource_path(
        "ifmerge_gui", "i18n", "locales", "zh.json").exists()
