...
### 改进
- ✅ **相似度矩阵显示所有IF对**：不仅显示超过阈值的相似度
  - 之前：只显示相似度≥80%的IF对
  - 现在：显示所有IF对的相似度（包括低于阈值的）
  - 用途：更全面地了解IF之间的关系

### 技术细节
- 新增 `SimilarityCalculator.build_full_similarity_matrix()` 方法
- 分组仍使用阈值过滤（只合并相似度≥阈值的IF）
- 矩阵输出使用完整相似度数据（显示所有IF对）

## [2.2.0] - 2026-01-26

### 重大变更 🚨
- ✅ **按模块合并输出**：相同模块的所有IF合并到一个文件中
  - 文件结构：`output/[モジュール]/グルーピング結果_[モジュール].xlsx`
  - 例：`output/SD/グルーピング結果_SD.xlsx` 包含所有SD模块的IF
- ✅ **新增列**：在D列和E列添加"モジュール"和"業務内容"
- ✅ **グルーピングID规则变更**：按模块+流水号编号
  - 旧格式：G001, G002, G003...
  - 新格式：FI001, SD001, SD002...（按模块前缀）
- ✅ **相似度矩阵使用文書管理番号**：行列标题改为文档管理编号
- ✅ **相似度矩阵按模块多Sheet输出**：
  - 一个模块一个文件
  - 不同场景放在不同sheet
  - Sheet命名：`[モジュール]_[業務内容]_類似度`

### 新增
- ✅ `MatrixExporter.export_module_matrices()` - 多sheet矩阵输出
- ✅ `MergeGrouper.assign_group_ids(module=...)` - 支持模块前缀
- ✅ CLI按模块组织处理流程
...
### 新增
- ✅ **相似度矩阵输出功能**：自动生成Excel格式的相似度矩阵
  - 在每个分类文件夹中生成`類似度マトリックス_[分类名].xlsx`
  - 下三角矩阵格式（对角线为「-」，上三角为空）
  - IF名作为行和列的标题
  - 相似度值以百分比格式显示（例：85.0%，保留1位小数）
- ✅ **新模块**：`ebs_merger/matrix_exporter.py` - 专门处理矩阵导出

### 改进
- ✅ 可视化：更容易查看和分析IF之间的相似度关系
- ✅ 百分比格式：更直观的相似度表示
- ✅ 报告：便于共享和报告相似度分析结果
- ✅ 文档：更新README和批处理脚本指南
